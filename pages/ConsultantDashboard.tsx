...
const ConsultantDashboard: React.FC = () => {
  ...
  const fetchRevenueData = async () => {
    try {
      ...
      if (!earnings || earnings.length === 0) {
        const emptyData = timePeriod === '7days' ? [
          { name: 'Mon', revenue: 0 },
          { name: 'Tue', revenue: 0 },
          { name: 'Wed', revenue: 0 },
          { name: 'Thu', revenue: 0 },
  ...
  const fetchUpcomingSessions = async () => {
    ...
    try {
      ...
      const upcomingBookings = response.filter((booking: Booking) => 
        ...
        <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-6">
          {statsLoading ? (
            ...
            [
              ...
              {
                label: 'Avg. Rating',
                value: profile.rating > 0 ? profile.rating.toFixed(1) : dashboardStats.averageRating,
          ...
          <div className="space-y-8">
            ...
            <div className="bg-white rounded-3xl p-6 border border-gray-100 shadow-sm">
              <div className="flex items-center justify-between mb-6">
                ...
                <button className="p-2 hover:bg-gray-50 rounded-xl transition-all"><Video size={20} className="text-gray-400" /></button>
              </div>
              <div className="space-y-3">
                {sessionsLoading ? (
                  ...
                  upcomingSessions.slice(0, 3).map((session) => (
                    <div key={session.id} className="flex items-center space-x-3 p-3 bg-gray-50 rounded-xl hover:bg-gray-100 transition-all cursor-pointer">
                      <div className="bg-blue-100 p-2 rounded-xl">
                        <Video className="text-blue-600" size={16} />
                      </div>
                      <div className="flex-1">
                        <p className="text-sm font-medium text-gray-900">
                          {new Date(session.date).toLocaleDateString()} at {session.time_slot}
                        </p>
                        <p className="text-xs text-gray-500">Session with Client</p>
                      </div>
